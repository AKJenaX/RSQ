...
class AssignmentRepositoryImpl : AssignmentRepository {
    companion object {
        private val _assignments = MutableStateFlow<List<Assignment>>(
            listOf(
                Assignment(
                    id = "ASGN-2024-01",
                    reportId = "SOS-911",
    ...
    override fun getAssignments(): Flow<List<Assignment>> = _assignments.asStateFlow()

    override fun getAssignmentById(id: String): Flow<Assignment?> = 
        _assignments.map { list -> list.find { it.id == id } }

    override fun getAssignmentsForVolunteer(volunteerId: String): Flow<List<Assignment>> =
        _assignments.map { list -> list.filter { it.volunteerId == volunteerId } }

    override fun getAssignmentsForReport(reportId: String): Flow<List<Assignment>> =
        _assignments.map { list -> list.filter { it.reportId == reportId } }

    override suspend fun createAssignment(assignment: Assignment) {
        _assignments.update { it + assignment }
    }
            ...
            list.map { assignment ->
                if (assignment.id == id) {
                    if (isValidTransition(assignment.status, status)) {
                        assignment.copy(status = status, updatedAt = System.currentTimeMillis())
                    } else {
                        assignment
                    }
                } else assignment
    ...
    private fun isValidTransition(from: AssignmentStatus, to: AssignmentStatus): Boolean {
        return when (from) {
            AssignmentStatus.AVAILABLE -> to == AssignmentStatus.ASSIGNED
            AssignmentStatus.ASSIGNED -> to == AssignmentStatus.IN_PROGRESS || to == AssignmentStatus.AVAILABLE
            AssignmentStatus.IN_PROGRESS -> to == AssignmentStatus.RESOLVED
            AssignmentStatus.RESOLVED -> false
        }
    }
}
